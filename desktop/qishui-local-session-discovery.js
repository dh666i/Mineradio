...
function qishuiPathKey(value, platform = process.platform) {
...
function qishuiRootHint(fullPath, appDataPath, localAppDataPath, kind, platform) {
  const resolved = path.resolve(fullPath);
  const bases = [
    [platform === 'darwin' ? 'Application Support' : 'Roaming', appDataPath],
...
function discoverQishuiClientDataRoots(options = {}) {
  ...
  const add = (value, kind, extra = {}) => {
    ...
    if (!resolved) return;
    const key = qishuiPathKey(resolved, platform);
    const exists = qishuiPathExists(resolved, fsImpl);
    const next = {
      path: resolved,
      kind: String(kind || 'detected'),
      hint: qishuiRootHint(resolved, appDataPath, localAppDataPath, kind, platform),
  ...
  if (platform === 'darwin' && homePath) {
    addKnownAndDetected(path.join(homePath, 'Library', 'Containers'), 'macos-container');
    addKnownAndDetected(path.join(homePath, 'Library', 'Group Containers'), 'macos-group-container');
  }
...
function discoverQishuiCookieStores(rootCandidate, options = {}) {
  ...
  const addStore = (databasePath, depth) => {
    ...
    const key = qishuiPathKey(databasePath, platform);
    if (result.stores.some(store => qishuiPathKey(store.cookieDbPath, platform) === key)) return;
