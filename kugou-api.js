...
const KUGOU_PLAYLIST_SEARCH_URL = 'https://mobiles.kugou.com/api/v3/search/special';
...
const kugouEntitySearchCache = createKugouTtlCache(80, 2 * 60 * 1000);
...
function requireSecureKugouUrl(targetUrl) {
  const url = new URL(targetUrl);
  if (url.protocol !== 'https:') {
    const error = new Error('KUGOU_INSECURE_TRANSPORT');
    error.code = 'KUGOU_INSECURE_TRANSPORT';
    throw error;
  }
  return url;
}
...
function requestText(targetUrl, opts, body) {
  ...
  return new Promise((resolve, reject) => {
    let u;
    try {
      u = requireSecureKugouUrl(targetUrl);
    } catch (error) {
      reject(error);
      return;
    }
...
async function kugouH5GatewayRequest(path, opts) {
  ...
  if (json && Number(json.status) === 0) {
    ...
    err.code = json.errcode || json.error_code || json.code || 'KUGOU_GATEWAY_FAILED';
    err.statusCode = Number(json.http_status || json.status_code) || 0;
    err.body = json;
    throw err;
  }
  return json;
...
async function getKugouLoginInfo(cookie) {
  ...
  if (auth.playbackReady) {
    try {
      profile = await fetchKugouProfileFromPlaylists(cookie, auth);
    } catch (error) {
      profileError = error;
    }
  }
  if (profileError && isKugouAuthInvalidError(profileError)) {
    return {
      provider: 'kugou',
      loggedIn: false,
      playbackReady: false,
  ...
  return {
    ...
    hasCookie: !!cookie,
    hasToken: !!auth.token,
    sessionValidated: !!(auth.playbackReady && !profileError),
    unavailable: !!profileError,
    loginCheckFailed: !!profileError,
  };
}

function isKugouAuthInvalidError(error) {
  if (!error) return false;
  const body = error.body && typeof error.body === 'object' ? error.body : {};
  const statusCode = Number(error.statusCode || body.http_status || body.status_code);
  const code = Number(error.code || body.errcode || body.error_code || body.code);
  if (statusCode === 401 || statusCode === 403 || code === 401 || code === 403) return true;
  const text = [
    error.message,
    body.error,
    body.errmsg,
...
function mapKugouPlaylistItem(item) {
  ...
  return {
    ...
    trackCount: Number(item.count || item.m_count || item.songcount || item.song_count || item.total || item.list_count || 0) || 0,
    creator: stripKugouHtml(item.nickname || item.username || item.user_name || item.list_create_username || ''),
  };
}

function mapKugouTypedPlaylist(item) {
  item = item || {};
  ...
  return {
    ...
    source: 'kugou',
    type: 'playlist',
    description: stripKugouHtml(item.intro || item.description || item.desc || ''),
    playCount: Number(item.playcount || item.play_count || item.listen_num || 0) || 0,
    creator: stripKugouHtml(item.nickname || item.username || item.user_name || item.list_create_username || ''),
    creatorId: String(item.suid || item.user_id || item.userid || ''),
    subscribed: false,
  };
}

async function handleKugouTypedSearch(keywords, type, limit, cookie, offset) {
  const auth = extractKugouAuth(cookie);
  ...
  if (!auth.playbackReady) {
    return {
      ...
      message: '请先完成酷狗登录，再搜索并打开酷狗歌单。',
    };
  }
  const query = stripKugouHtml(keywords);
  if (!query) {
    return {
      provider: 'kugou',
      source: 'kugou',
  ...
  const cacheKey = ['typed', searchType, query.toLowerCase(), limit, offset].join('|');
  return kugouEntitySearchCache.wrap(cacheKey, 2 * 60 * 1000, async () => {
    const pageSize = 50;
    ...
    while (rawItems.length < localStart + limit) {
      const url = new URL(KUGOU_PLAYLIST_SEARCH_URL);
      ...
      if (!json || Number(json.status) === 0 || Number(json.errcode || 0) !== 0) {
        const error = new Error(json && (json.error || json.errmsg || json.message) || 'KUGOU_TYPED_SEARCH_FAILED');
        error.code = 'KUGOU_TYPED_SEARCH_FAILED';
        throw error;
      }
      const data = json && json.data || {};
      const pageItems = Array.isArray(data.info)
        ? data.info
        : (Array.isArray(data.list) ? data.list : []);
      total = Number(data.total || json && json.total || total) || total;
    ...
    const items = pageItems.map(mapKugouTypedPlaylist).filter(item => item.id && item.name);
...
module.exports = {
  ...
  _test: {
    apiEndpoints: Object.freeze({
      search: KUGOU_SEARCH_URL,
      playlistSearch: KUGOU_PLAYLIST_SEARCH_URL,
      mobilePlayback: KUGOU_PLAY_MOBILE,
    }),
    requireSecureKugouUrl,
    isKugouAuthInvalidError,
    mapKugouTypedPlaylist,
