...
const protectedCredentialStore = require('./lib/protected-credential-store');
...
function readQishuiOAuthFileConfig() {
  ...
  for (const file of candidates) {
    try {
      ...
      const parsed = protectedCredentialStore.readJson(file, {});
...
const qishuiEntitySearchCache = createTtlCache(60, 2 * 60 * 1000);
...
function qishuiAccessTokenInfo() {
  ...
  try {
    if (fs.existsSync(file)) {
      const token = normalizeQishuiToken(protectedCredentialStore.readString(file));
...
function saveQishuiAccessToken(value) {
  ...
  const file = qishuiTokenFile();
  protectedCredentialStore.writeString(file, token);
...
function clearQishuiAccessToken() {
  const file = qishuiTokenFile();
  protectedCredentialStore.remove(file);
  clearQishuiRuntimeCaches();
  ...
  return qishuiWebLibraryCache.wrap(cacheKey, 90 * 1000, async () => {
    ...
    const tryRead = async (label, apiPath, params, requestOpts) => {
      ...
      try {
        ...
        if (isQishuiAuthInvalidError(err)) authExpired = true;
    ...
    return {
      ...
      configured: !authExpired,
      webSession: !authExpired,
      authExpired,
...
async function handleQishuiStatus(cookieText) {
  ...
  try {
    ...
    if (library && library.authExpired) {
      status.configured = !!status.tokenConfigured;
      status.webSession = false;
      status.cookieReady = false;
      status.loggedIn = !!status.tokenConfigured;
      status.playbackMode = 'recommend-match';
      status.authExpired = true;
      status.reauthRequired = true;
      status.stale = true;
      status.error = 'LOGIN_EXPIRED';
      status.message = '汽水音乐登录已失效，请重新导入官方客户端登录态。';
      return status;
    }
    ...
    status.sessionValidated = !status.libraryErrors.some(item => /^me:/i.test(String(item || '')));
    status.unavailable = !status.sessionValidated;
    status.loginCheckFailed = !status.sessionValidated;
  } catch (err) {
    if (isQishuiAuthInvalidError(err)) {
      status.configured = !!status.tokenConfigured;
      status.webSession = false;
      status.cookieReady = false;
      status.loggedIn = !!status.tokenConfigured;
...
async function handleQishuiTypedSearch(keywords, type, limit, cookieText, offset) {
  const searchType = normalizeText(type).toLowerCase();
  ...
  if (!status.webSession) {
    return {
      ...
      webSession: false,
      requiresLogin: true,
      error: 'QISHUI_AUTH_REQUIRED',
      reason: 'missing_web_session',
      message: '请先导入本机汽水音乐登录态，再搜索并打开汽水歌单。',
    };
  }
  ...
  return qishuiEntitySearchCache.wrap(cacheKey, 2 * 60 * 1000, async () => {
    ...
    const items = extractQishuiPlaylistCards(json)
      .map(item => ({ ...item, provider: 'qishui', source: 'qishui', type: 'playlist' }))
      .slice(0, limit);
    const data = json && json.data || json || {};
    const resultData = pickObject(data.search_result, data.searchResult, data);
    const rawItems = extractQishuiPcSearchItems(json);
    const rawCount = Math.max(items.length, rawItems.length);
    ...
    const hasMoreFlag = resultData.has_more != null
      ? resultData.has_more
      : (resultData.hasMore != null ? resultData.hasMore : data.has_more);
    const hasMore = typeof hasMoreFlag === 'boolean'
      ? hasMoreFlag
      : (Number(hasMoreFlag) > 0 || !!nextCursor || (rawCount >= limit && (!reportedTotal || offset + rawCount < reportedTotal)));
    return {
      provider: 'qishui',
      source: 'qishui',
      type: searchType,
      ...
      webSession: true,
      items,
      total: Math.max(reportedTotal, offset + rawCount),
      rawCount,
      offset,
      limit,
...
module.exports = {
  ...
  qishuiUnavailable,
  _test: {
    isQishuiAuthInvalidError,
    extractQishuiPlaylistCards,
