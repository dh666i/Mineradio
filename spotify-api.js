...
function readStoredSpotifyToken() {
  ...
  try {
    ...
    const raw = protectedCredentialStore.readJson(file, {});
    return {
      file,
...
function writeJsonFile(file, payload) {
  protectedCredentialStore.writeJson(file, payload);
}
...
function clearSpotifyToken() {
  const file = getSpotifyTokenFile();
  if (file) protectedCredentialStore.remove(file);
  spotifyProfileCache = { value: null, at: 0, promise: null };
...
function mapSpotifyAlbum(item) {
  ...
  const releaseDate = normalizeText(item.release_date);
  const parsedPublishTime = releaseDate ? Date.parse(releaseDate) : 0;
  const songCount = Number(item.total_tracks) || 0;
  return {
    provider: 'spotify',
    source: 'spotify',
    type: 'album',
    ...
    publishTime: Number.isFinite(parsedPublishTime) ? parsedPublishTime : 0,
    releaseDate,
    albumType: normalizeText(item.album_type),
    albumGroup: normalizeText(item.album_group),
...
async function handleSpotifyTypedSearch(keywords, type, limit, offset) {
  keywords = normalizeText(keywords);
  type = normalizeText(type).toLowerCase();
  limit = Math.max(1, Math.min(SPOTIFY_SEARCH_LIMIT_MAX, Number(limit) || 24));
  offset = Math.max(0, Number(offset) || 0);
  ...
  const config = getSpotifyConfig();
  let status = config;
  if (type === 'playlist') status = await handleSpotifyStatus();
  if (!status.capabilities || !status.capabilities.search || (type === 'playlist' && !status.loggedIn)) {
    return {
      provider: 'spotify',
      source: 'spotify',
      type,
      ...
      requiresLogin: true,
      error: 'SPOTIFY_AUTH_REQUIRED',
      reason: type === 'playlist' ? 'spotify_user_login_required' : 'missing_spotify_auth',
      missing: status.oauthMissing && status.oauthMissing.length ? status.oauthMissing : status.missing,
      message: type === 'playlist'
        ? '请先连接 Spotify 账号，再搜索可由当前账号打开的 Spotify 歌单。'
        : status.message,
    };
  }
  if (!keywords) {
  ...
  const cacheKey = [
    ...
    offset,
    status.market,
    status.userId || (status.tokenConfigured ? 'user' : 'client'),
  ].join('|');
  return cacheWrap(spotifySearchCache, cacheKey, 2 * 60 * 1000, async () => {
    const items = [];
    let rawCount = 0;
    ...
    let hasMore = false;
    let requestCount = 0;
    while (items.length < limit && requestCount < 4) {
      requestCount += 1;
      const pageLimit = type === 'playlist'
        ? SPOTIFY_SEARCH_LIMIT_MAX
        : Math.min(SPOTIFY_SEARCH_LIMIT_MAX, limit - items.length);
      const json = await spotifyGet('/search', {
        q: keywords,
        type,
        market: status.market,
      ...
      const collection = json && json[type + 's'] || {};
      const pageItems = Array.isArray(collection.items) ? collection.items : [];
      total = Number(collection.total) || total;
      rawCount += pageItems.length;
      hasMore = !!collection.next;
      if (type === 'album') {
        items.push(...pageItems.map(mapSpotifyAlbum).filter(Boolean));
      } else {
        const profile = { id: status.userId };
        pageItems.forEach(item => {
          const ownerId = normalizeText(item && item.owner && item.owner.id);
          if (!item || (ownerId !== status.userId && !item.collaborative)) return;
          const mapped = mapSpotifyPlaylist(item, profile);
          if (mapped) items.push(mapped);
        });
      }
      if (!pageItems.length || !hasMore) break;
    ...
    return {
      ...
      total: Math.max(total, offset + rawCount),
      rawCount,
      offset,
      limit,
      nextOffset: offset + rawCount,
      hasMore,
      filtered: type === 'playlist',
      filterReason: type === 'playlist' ? 'openable_by_current_account' : '',
    };
  });
}
