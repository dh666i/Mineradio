# Changelog

## v3.0.2 - 2026-08-13

- 移除搜索框下方常驻的“综合 / 单曲 / 歌手 / 专辑 / 歌单”类型栏，保留综合结果内的分类入口与 DIY 模式的平台来源栏；分类详情新增明确的返回综合搜索入口。
- 综合搜索按账号状态重新排序：当前选中的有效登录账号来源固定优先，其他有效登录来源其次，未登录公共来源最后；单曲、歌手、专辑、歌单以及批量播放去重共享同一优先级。
- 搜索结果过滤缺少真实平台 ID、明确下架、版权不可用和已确认无法取链的音源；媒体死链会按账号及会员作用域短期缓存，系统自动播放拦截不会被误判为无效歌曲。
- QQ 音乐、酷狗音乐和汽水音乐搜索在过滤无效条目后继续使用上游原始数量推进分页，避免加载更多时重复、跳页或提前结束。
- 修复在分类详情中重复点击当前来源标签后内部状态与画面不一致的问题；现在会回到相同关键词的综合结果。
- 网易云代理链的 `ip-address` 锁定到 `10.5.0`，修复生产依赖中的地址解析与 SSRF 边界漏洞；生产依赖审计恢复为零告警。

## v3.0.1 - 2026-07-29

- 五平台搜索结果新增“综合 / 单曲 / 歌手 / 专辑 / 歌单”分类；歌手覆盖网易云与 QQ 音乐，专辑覆盖网易云与 Spotify，歌单覆盖全部五个平台，结果带来源标识并支持详情和独立分页。
- Spotify 首次登录可直接在应用内保存 Client ID 后跳转官方 PKCE 授权，不收集 Client Secret；未配置的新安装不再出现无法操作的登录入口。
- 完善网易云与 QQ 音乐歌手详情分页；歌手页新增“播放前100首”，按需补齐热门歌曲并避免一次载入过大的完整曲库。
- 补齐网易云音乐、QQ 音乐、酷狗音乐、汽水音乐与 Spotify 五平台账号状态和桌面登录入口；持久化敏感凭据由 Electron 主进程加密或受保护保存，账号状态接口不再返回明文 Cookie、Token 或 Client Secret。
- 加固平台登录与凭据生命周期：登录窗口仅允许可信 HTTPS 页面，重新授权会清理旧会话，凭据写入失败不再误报成功，并区分确定过期与临时网络失败；无法安全加密的历史明文凭据会被隔离。
- 五平台单曲搜索统一接入独立分页、单来源失败重试与来源状态；本地结果筛选支持歌曲名、歌手名的全拼及首字母匹配，拼音能力不可用时自动回退普通匹配。
- 补齐酷狗、汽水与 Spotify 的歌单、歌词和播放分发；不可播、权限不足、登录过期与平台只返回元数据时会明确提示，并可在限定次数和时间内自动匹配其它平台的可播版本。
- 搜索框支持粘贴五平台分享歌单链接并本地导入；网易云、QQ 与 Spotify 复用平台歌单接口，酷狗支持 `gcid` 解码、公开接口与 H5 `specialid` 回退，汽水支持从公开分享页读取曲目及部分结果。
- 音量面板改为点击按钮后展开、再次点击或点击外部关闭，不再因鼠标悬停意外弹出；当前队列关闭按钮调整为紧凑图标。
- 新增安全媒体代理：逐跳校验 DNS 与目标 IP、固定已验证解析结果、限制重定向和媒体 MIME，支持 Range、HEAD、连接/空闲/总超时及下游断开取消；汽水加密音频继续在受限缓存中完成下载与解密。
- 播放恢复增加事务归属、20 秒总截止时间、候选次数和队列跳过上限；过期媒体事件不会干扰新歌曲，`error` 或 `stalled` 时每首最多刷新一次地址，关闭的 AudioContext 会绑定当前媒体后重建。
- “关于”页新增安全缓存清理，只删除网络缓存与节奏分析缓存，不影响账号、歌单、设置和用户资源。
- 新增 blockmap 差量更新：存在可复用的旧安装包和块清单时，仅通过 HTTP Range 下载变化块；复用块逐块校验，拼装结果继续执行完整安装包摘要校验。
- 差量条件不满足、块校验失败或线路不支持 Range 时自动回退完整安装包下载，不影响原有更新流程；可通过 `MINERADIO_UPDATE_DIFFERENTIAL=0` 停用。
- 完整下载完成后保存对应 blockmap 边车，供下一版本差量升级复用；新增核心与离线集成测试以及本地真实安装包验证脚本。
- 移植上游 `v2.0.1` 的首次启动白色高亮与 3D 歌单架层级修复，移除会遮挡歌词的舞台地面反射。
- 移植上游 `v2.0.2` 的有界音源恢复：限制自动换源时间、候选次数与队列跳过次数，失败候选自动回滚，并隔离过期媒体事件。
- 移植上游 `v2.0.3` 的透明对象层级修复：多行歌词的内部图层随舞台层级同步，隐藏光标后已选中的 3D 歌单卡片不再持续遮挡歌词。
- 收紧渲染层内容策略，移除运行时远程脚本与动态执行；外部封面统一通过受限图片代理，系统媒体与壁纸窗口只接收已校验的同源图片，并修复内嵌封面、旧端口听歌记录及远程背景视频的兼容边界。
- 诊断日志新增 Cookie 数组、OAuth Token、Bearer 凭据与常见登录字段脱敏；启动时会清理历史日志中遗留的敏感值，同时保留错误状态和排查上下文。
- Electron 启动时新增本地 HTTP 就绪探测和主窗口有界重试，降低服务已监听但首页尚未可访问时出现空白窗口的概率。
- 已打包应用禁止使用本地或任意 HTTP manifest 覆盖更新元数据，继续只信任仓库配置生成的 GitHub 更新信息；源码开发仍保留本地更新验证能力。
- 更新说明会过滤隐藏注释、URL 与下载线路标题；清理已停用的 JSON 资源补丁写入代码，旧补丁接口继续固定返回 `410`，不影响 blockmap 差量更新。
- QQ 会员状态改为账号级官方探测与凭证指纹缓存，避免旧 Cookie 或模糊字段误判；QQ 取链按音质并发探测可播候选，并设置 vkey、探测和整次请求的明确时间预算。
- Windows 默认发布构建改为无签名安装包，保留显式自签构建命令；应用内更新继续校验 GitHub 官方元数据、安装包大小及完整摘要。
- 酷狗账号相关搜索与取链端点统一改为 HTTPS，并在请求底层拒绝任何明文 HTTP 回退，避免 Cookie 或 Token 通过不安全传输发送。
- Windows 构建链升级到 electron-builder `26.15.7`，纳入 v26 维护通道的 NSIS 安装可靠性与传递依赖补丁；正式包继续排除全部开发依赖。
- 新增 macOS x64 与 Apple Silicon 的 DMG/ZIP 云端预览构建；隔离 Windows 专属 D3D、任务栏、壁纸和安装器更新能力，并适配 macOS 图标、Dock、托盘、路径大小写及常用会话目录。
- macOS 预览包使用无需开发者账号的 ad-hoc 本地完整性签名，但未使用 Apple Developer 身份签名或公证；实际播放、媒体键、多屏与 Gatekeeper 体验仍需 Mac 真机验证。

## v1.5.4 - 2026-07-22

- 新增切歌过渡设置：支持关闭、默认无缝衔接，以及 3 秒、5 秒和 8 秒等功率交叉淡化，旧版音频设置会自动补齐默认值。
- 顺序播放会提前准备下一首网易云歌曲，并通过双音频槽共享频谱、节拍分析、均衡器和主音量输出，降低自然切歌时的等待与静音间隙。
- 播客、本地音乐、QQ 音乐、私人 FM、单曲循环、随机播放、倍速和“播完本首停止”保持普通切歌，避免不适用场景出现错误过渡。
- 完善候选音源失败、启动超时、队列重排、手动切歌、暂停、进度拖动、睡眠定时和旧异步请求迟到时的恢复逻辑，防止错歌、重复推进、双音轨或无法暂停。
- 修复搜索框与桌面标题栏之间的悬停断层；鼠标离开窗口、窗口失焦或隐藏时会收起搜索等临时模块。
- 恢复上次歌曲和播放位置时立即重建中央视觉，不再需要先点击播放才能看到封面粒子与歌词舞台。
- 优化歌单展开详情、操作区、歌曲列表和滚动条样式，移除重复标题与嵌套玻璃面板，统一紧凑布局。
- 将设置中的“更新与数据”整理为“关于”，保留更新、备份和诊断入口，并增加功能与优化建议反馈二维码。

## v1.5.3 - 2026-07-21

- 主程序中央歌词新增“自动 / 单句 / 三句”布局，设置可持久化，并随视觉配置存档导入导出。
- 自动模式在竖屏显示上一句、当前句和下一句；横屏保持聚焦当前句，长歌词横屏最多两行、竖屏最多三行。
- 优化中英文、CJK 标点、组合字符和 emoji 的智能换行，逐字高亮可连续跨越换行后的各视觉行。
- 翻译与罗马音仅附着当前歌词；当前歌词占用空间较多时自动弱化或隐藏上下文，避免重叠。
- 增加竖屏视锥自适应和动态尺寸保护，修复多行歌词在呼吸、节拍动画、窗口横竖切换及换句重建时的裁切和闪断。
- 压缩歌词可读性层和溢光纹理占用，并优化超长歌词分段算法，降低切句时的显存和计算压力。

## v1.5.2 - 2026-07-21

- 修复主窗口在跨屏拖动时被限制在当前显示器的问题，登录窗口也可自由移到其他显示器。
- 新增主窗口状态保存，重启后恢复上次所在显示器、窗口坐标、尺寸与最大化状态。
- 完善多显示器异常恢复，兼容负坐标、上下排列和不同 DPI；显示器断开、重新排列或分辨率变化后，会将不可操作的窗口恢复到可见区域。
- 优化音乐库播放队列布局，歌曲名与歌手分行显示，长文本自动省略，操作按钮不再遮挡内容，并移除异常的横向滚动条。
- 精简歌单展开详情，移除与歌单卡片重复的封面、标题、曲目数和嵌套玻璃背景，操作区改为紧凑的播放与收起控件。
- 更新网络请求、请求解析和构建链路中的上游依赖安全补丁，修复已公开的拒绝服务与原型污染风险。

## v1.5.1 - 2026-07-20

- 沉浸模式改为直接进入应用全屏，默认隐藏搜索、账号区、视觉控制台、歌单面板和播放器控制栏；鼠标靠近对应屏幕边缘时再显示完整的原版界面元素。
- 移除控制栏自动隐藏后的简洁迷你播放器样式，统一保留原版完整控制栏，并在退出沉浸模式后恢复进入前的面板与控制栏状态。
- 重做 Electron 全屏状态同步与窗口恢复流程，修复沉浸模式、Esc 和窗口按钮切换全屏时可能出现的状态反复、退出不完整及原窗口尺寸丢失问题。
- 优化启动与窗口重新激活后的页面选择：存在已恢复歌曲时显示上次歌曲、队列和播放位置，没有可显示歌曲时默认进入 Home，避免停留在空白播放器页面。
- 调整右上角窗口控制区的内边距，使按钮与窗口边缘保持更舒适的视觉距离。
- 修复搜索页在窄窗口下的横向溢出、工具栏和歌曲信息压缩异常、更多操作省略号裁切及输入框焦点边框突兀问题，并优化结果计数文案和滚动条样式。
- 打开设置、登录等弹窗时会先安全退出沉浸模式，避免弹层被全屏沉浸界面隐藏。

## v1.5.0 - 2026-07-20

- 新增网易云发现页与分类浏览，支持热门歌单、新碟、新歌和歌单分类分页；歌曲、歌手、专辑、歌单搜索改为独立类型并支持完整分页。
- 搜索结果与歌单详情支持播放全部、完整载入和稳定取消旧请求；登录过期、空映射页、部分结果及上游分页游标均有明确处理。
- 网易云歌单补齐完整曲目加载、收藏与取消收藏，以及本人歌单名称、简介、标签和隐私设置编辑。
- 新增 10 段均衡器、音效预设、基础响度与峰值保护、输出设备选择和网易云下一首地址预取，音效关闭时保持原始直通链路。
- 播放队列持久化上限提升至 5000 首，大型队列使用固定窗口分页并保留当前歌曲，批量删除、排序和跨页定位保持一致。
- 主窗口、桌面歌词和壁纸窗口启用 Electron 沙箱；收紧登录窗口导航、权限和 IPC 调用来源，阻止非可信页面调用桌面能力。
- 增加脱敏持久诊断日志、渲染器与子进程异常记录、统一源码检查脚本和 Windows CI 构建检查。
- 修复设置中心继承普通弹窗宽度上限导致的中文逐字换行、音频页横向滚动和更新按钮竖排，并统一音乐库工具按钮尺寸与换行行为。
- 补齐旧式点击目标与图标按钮的键盘可达性、焦点样式和辅助名称。

## v1.4.0 - 2026-07-19

- 重排 Home 信息层级：每日推荐保持主入口，并补齐我的歌单、红心歌曲、继续听、接下来播放和推荐内容入口。
- 网易云与 QQ 音乐歌曲搜索支持独立分页、加载更多、来源标识和单来源失败重试；新增行级快捷操作、专辑详情和歌手专辑。
- 播放队列支持安全持久化与暂停恢复、拖动和键盘排序、多选移除、撤销、随机播放避开当前歌曲，以及保存为网易云歌单。
- 增加本人普通网易云歌单的重命名、删除、移除歌曲和完整曲序同步；服务端再次校验歌单所有权与曲目集合。
- 歌词新增翻译与罗马音副行，保留网易云 YRC 和 QQ QRC 逐字时间；桌面歌词暂停时保持当前行并独立显示副行。
- 播客增加前后 15 秒、倍速和睡眠定时；进度条、队列和弹窗补齐键盘与无障碍状态。
- 新增设置中心，统一播放质量、控制栏、减少动态、性能后台策略、快捷键和更新入口。
- 更新下载支持真实取消、线路、速度和预计剩余时间状态；任务创建期间的取消请求会在任务 ID 返回后立即执行。
- 网易云与 QQ 官方网页登录 Cookie 改由 Electron 主进程直接写入本地服务，渲染进程只接收脱敏账号状态；明确区分部分登录、过期和服务不可用。
- 增加请求超时与网络错误分类、主窗口导航限制、登录与更新 IPC 来源校验，以及更新取消集成测试。
- Windows 签名流程固定到既有自签证书指纹，避免误换证书破坏已安装版本的更新校验。

## v1.3.2 - 2026-07-19

- 修复更新安装包候选线路被重复展开的问题，线路数量恢复为有效候选，不再出现镜像地址嵌套。
- 下载前并行测试各线路的小范围数据并优先选择响应更快的线路；测速失败时仍保留原有回退线路。
- 下载持续低速时自动切换到下一条未尝试线路，更新弹窗同时提供手动切换线路操作。
- 安装包下载完成后继续执行文件名、大小、摘要和 Authenticode 签名校验。

## v1.3.1 - 2026-07-19

- 顶部更新入口仅在成功检测到 GitHub 最新版本高于本机版本时显示；同版本、检查中和检查失败时保持隐藏。

## v1.3.0 - 2026-07-19

- Windows 发布构建默认生成并复用本机自签名 Authenticode 证书，签名主程序、辅助程序、卸载器和安装器，并校验 RFC 3161 时间戳。
- 应用技术标识迁移到 `com.dh666i.mineradio`，用户数据迁移到独立目录，同时固定沿用旧版 NSIS 升级 GUID；旧配置和用户资源会安全复制，旧目录保留，但迁移成功的明文登录凭据会被清除。
- 主窗口、网易云登录窗口、QQ 音乐登录窗口和 NSIS 安装器支持 Per-Monitor V2 DPI；跨不同缩放显示器时动态调整窗口边界。
- 接入 Windows Media Session、系统媒体键、锁屏歌曲信息和任务栏缩略图播放控制，支持播放、暂停、上一首、下一首和进度跳转。
- 关闭主窗口时默认隐藏到系统托盘，托盘支持恢复窗口、播放暂停、上一首、下一首和明确退出。
- 更新检查区分最新、可更新和检查失败；更新清单仅从 GitHub 官方 HTTPS 与 Releases API 获取，安装包必须匹配精确名称、大小和摘要，快速补丁入口默认关闭。
- 打开更新安装包前验证 Authenticode 状态、时间戳并比较当前程序与安装包的证书指纹，阻止签名损坏、未签名或签名者不一致的文件。
- 网易云和 QQ 音乐登录 Cookie 使用 Electron `safeStorage` 加密落盘，并自动迁移旧明文凭据。
- 增加升级前设置备份、最近备份恢复和脱敏诊断导出；诊断信息不包含 Cookie、Token 或本机绝对路径。
- 清理界面、安装器、可执行文件元数据和发布文档中的版本类型后缀，产品名统一为 Mineradio。

## v1.2.0 - 2026-07-18

- 重排 Home 信息架构：每日推荐聚焦主操作与完整列表入口；“接着听”改为跨栏封面卡片并填满剩余空间，减少嵌套列表、文字拥挤和大面积空白。
- 清理仓库中的历史支持、收款、宣传和过期 AI 交接材料，仅在 NOTICE 中保留许可证要求的最低限度版权来源声明。
- 将 Home 施工区升级为网易云每日推荐：首页展示日期与封面，并提供完整推荐列表。
- 每日推荐支持播放全部、随机播放和从指定歌曲开始播放；登录过期、请求失败和跨日缓存都有明确状态。
- 接入网易云私人 FM 独立队列，支持自动预取和队尾补歌；FM 不受普通单曲循环或随机模式干扰。
- 搜索结果新增播放全部、批量加入队列和随机播放；跨网易云/QQ 音乐按歌曲、歌手和版本去重，网易云登录时优先使用网易云结果。
- 新增每日推荐、私人 FM 与 FM trash 本地 API，并修复网易云认证过期时的状态处理。
- 将网易云 API 的媒体元数据解析依赖锁定到已修复版本，排除畸形 ASF 数据导致进程死循环的已知漏洞。
- 统一安装器与可执行文件的 Mineradio 产品信息，并将 GPL、来源和隐私安全文档一并打包。
- 修复卸载器残留应用资源目录的问题；只递归清理经过安装标记验证的应用专属子目录，根目录中的无关文件仍会保留。
- 本地服务默认只监听 `127.0.0.1`，增加 Host、Origin 和 Fetch Metadata 校验，降低局域网暴露、跨站请求与 DNS rebinding 风险。
- 异步推荐播放增加播放意图保护，避免慢请求覆盖用户后来启动的歌曲。
- 更新项目归属、发布目标和应用内更新源到 `dh666i/Mineradio`，并保留原项目及 GPL-3.0 归属说明。

## v1.1.1

- P0 installer safety fix: installation now defaults to the first available non-C drive from `D:\Mineradio` through `Z:\Mineradio`; it falls back to `C:\Mineradio` only when no D-Z drive exists.
- The installer now forces the target path into a dedicated `Mineradio` folder, blocks non-empty non-Mineradio-owned targets, and blocks C drive installs when a D-Z drive is available.
- Existing registered installs in a dedicated `...\Mineradio` folder are adopted in place so users can run the new setup over their current installation and receive the safe uninstaller.
- Existing dedicated `...\Mineradio` folders that already contain Mineradio files can be overwritten even if they were created before the new safety marker existed.
- The new uninstaller no longer removes the whole installation root or app subfolders recursively; it deletes only known top-level Mineradio/Electron files and leaves unrelated files in place.
- Legacy uninstallers without the new safety marker are skipped during install; the setup deletes only the old `Uninstall Mineradio.exe` file and registry entry so it cannot indirectly trigger an older unsafe uninstall routine.
- Installer/uninstaller safety fixes require the full setup package; quick patch JSON files must not be used as the only delivery path for this issue.

## v1.1.0

- 纯净安装发布版：从当前 `resources/app` 可信源码重新构建 1.1.0 安装包，旧打包产物、旧备份包和旧安装包不再作为发布来源。
- 重要安全说明：`v1.0.10` 及更早旧安装包不再建议继续安装或传播，请先隔离旧安装包；本次不提供从 `v1.0.10` 到 `v1.1.0` 的软件内本地更新，请到 GitHub Release 手动下载 `v1.1.0` 进行纯净安装。
- 默认视觉参数更新：将 `默认测试.json` 作为首次启动默认用户存档和软件内视觉参数默认值；新用户首次启动即使用同一份快照，用户存档中自动预置「默认测试」。
- 默认视觉快照包括：`emily` 视觉预设、封面粒子分辨率 `1.55`、电影镜头强度 `0.5`、歌词发光 `0.28`、高亮色 `#fac900`、发光色 `#008aff`、UI/Home/视觉图标白色、3D 歌单架静态常驻、播客歌单默认关闭、侧向角度 `-15`。
- 3D 歌单架新增内容开关：可关闭播客歌单，可选择是否把「我的歌单」与「收藏歌单」合并成一条连续滚动列表。
- 3D 歌单架交互修复：动态/静态详情页绑定分流，静态详情页跟随封面粒子世界轴，动态详情页跟随镜头；常驻歌单架未命中时不再长期挡歌词，命中卡片时才浮起到前景。
- 3D 歌单详情页修复：详情页打开后保持更完整的页面显示和中心高亮行；修复固定/常驻状态下详情页被歌词或卡片遮挡、动态镜头触发区异常、滚轮高亮不同步等问题。
- 歌词透明度修复：详情页选歌/切歌时新词不再突然跳亮；详情页打开时歌词保持可读，但降低发光层并保持在详情页下层，避免遮挡中心高亮行。
- 高级设置新增性能项：后台策略支持自动优化、保持运行、停止释放；画质档位支持低/中/高/超高；直播后台保持开启后不再进入低占用暂停。
- 用户存档修复：应用用户视觉存档后会提交播放态视觉预设，跳转歌曲、切歌不再回退到上一个视觉预设。
- 全屏/控制台修复：视觉预设热键按钮不再被全屏 DIY 悬浮入口遮挡；视觉控制台隐藏按钮会跟随一起隐藏，不再残留一个孤立标记。
- 更新 Electron / electron-builder 依赖，修复已知高风险构建依赖告警。
- 修复软件内更新失败后的循环体验：快速补丁失败后不再自动反复拉完整安装包，完整安装包下载完成后也不再自动打开，需用户手动确认。
- 更新下载会复用本地已校验通过的安装包，避免重复下载同一文件。

## v1.0.10

- 重做桌面歌词视觉与交互：保持软件内歌词质感，优化白底可读性、长歌词滚动、锁定穿透、电影震动与最小化状态一致性。
- 修复安魂预设歌词位置无法调整的问题，当前嘴部歌词原位置作为原点，景深改为按视角前后移动。
- 歌词布局新增上下角度与左右角度调节，并会随本地布局和用户存档保存。
- 歌词镜头绑定开启时避让右侧 3D 歌单架，安魂预设也同步处理，避免遮挡歌单架操作。
- 新增方向键上下调节音量，每次 5%，输入框和滑条聚焦时不会误触。
- 调整更新包规则：补丁包命名使用 `旧版本→新版本` 表达，且以后只为低于新版的最近 4 个版本生成快速补丁。

## v1.0.9

- 修复安装包深色界面中部分文字仍为黑色导致看不清的问题，安装器改为浅色高对比界面。
- 安装包支持用户自由选择安装目录；默认仍优先 `D:\Mineradio`，选择盘根目录时会自动补全 `Mineradio` 文件夹。
- 修复软件可重复打开的问题，重复启动时会聚焦当前已运行的窗口。
- 修复软件每次启动都会自动创建/刷新桌面快捷方式的问题。

## v1.0.8

- 修复 QQ 音乐登录后头像/昵称与播放授权状态不同步的问题，歌单能读但歌曲无法播放时会优先识别真实播放票据缺失。
- Home 左侧大卡片改为施工提示，并支持在 Home 页手动展开播放器控制台。
- 视觉预设顺序调整为 emily、安魂、星河、唱片、星球、滚筒、虚空；安魂十字架选中态保持同款蓝色。
- 视觉控制台新增 4 个槽位的“用户存档”，可保存粒子、颜色、滑条和歌词外观习惯，并支持卡片内直接命名。
- 修复重启后歌词预设/自定义颜色没有立即恢复到舞台歌词的问题。
- 播放与暂停增加音频淡入淡出，降低手动启停时的突兀感。

## v1.0.7

- 反正没什么人看，布想写日志了

## v1.0.6

- 反正没什么人看，布想写日志了

## v1.0.5

- 优化 GitHub 更新下载：默认优先尝试国内加速线路，失败后自动切换下一条线路，最后兜底 GitHub 直连。
- 更新弹窗新增下载速度、已下载大小、预计剩余时间、当前线路和线路尝试次数显示。
- 更新失败时保留具体失败原因和已尝试线路数量，方便用户判断是超时、DNS、HTTP、校验失败还是资源未同步。
- 下载完整安装包和快速补丁都会校验 release digest，避免镜像缓存异常导致坏包被应用。
- 修复旧版本快速补丁匹配过宽的问题，确保 `1.0.0`、`1.0.1`、`1.0.2`、`1.0.3` 只会匹配各自对应的补丁包。

## v1.0.4

- 优化最小化/隐藏窗口时的后台占用：只有真正最小化、不可见或文档隐藏时进入深度低占用；可见但失焦、副屏显示时保持正常帧率和电影镜头。
- 修复桌面全屏模式下 3D 视觉画布尺寸不同步导致的裁切/偏上问题；全屏、退出全屏和窗口 resize 后会连续刷新主渲染视口。
- 优化播放器控制台自动隐藏时的残影：隐藏状态不再保留玻璃背景、滤镜和阴影，同时保留显示时的原有玻璃质感。
- 视觉控制台新增“控制台玻璃色差”滑条，并给视觉控制台悬浮按钮增加贴边自动隐藏按钮。
- 左侧“我的歌单”详情页改为分批加载更多歌曲，放大“加载更多”按钮，详情头部支持滚动置顶和回到顶部。
- 沉浸模式恢复左侧歌单栏、3D 歌单架和默认封面渐变背景的可用性。

## v1.0.3

- 重构视觉预设控制台的信息结构，新增 `预设 / 外观 / 歌词 / 动态 / 高级` 五个分区，减少功能堆叠带来的操作压力。
- 优化色轮和封面取色面板的自适应定位，弹层会贴近触发按钮并限制在当前窗口内，降低不同分辨率下错位或越界的概率。
- 修复背景颜色状态混淆问题：纯黑色现在代表真正的黑色背景，封面渐变改为独立模式，控制台会明确显示“封面渐变”。
- 保留并优化根据专辑封面生成背景渐变的功能，避免点击默认时才触发、左侧色块却显示黑色的误导。

## v1.0.2

- Add collect-to-playlist and play-next actions to artist detail songs.
- Publish quick patches for `1.0.1 -> 1.0.2` and `1.0.0 -> 1.0.2`, plus a full installer for cross-version upgrades.

## v1.0.1

- Smooth Emily visual loading and track-change transitions.
- Remove the forced loading-state jump, shorten the Emily cover color mix, and fade cover depth in gradually.
- Publish a `1.0.0 -> 1.0.1` quick patch so installed users can receive an update prompt.

## v1.0.0

- 调整播放器控制台显隐规则：播放、切歌、天气电台开播不再主动弹出底部控制台；只有底部手柄/热区唤出时显示，鼠标移出热区后快速收起。
- 修复播放器控制台进度条不随播放推进的问题：进度条和时间显示现在会优先使用真实音频时长，并在浏览器暂未返回 duration 时使用歌曲 metadata 兜底。
- 修复网易云 / QQ 扫码成功但登录态不落地的问题：网易云不再因资料接口短暂无 profile 立刻清空 cookie，QQ 登录支持新式 `psrf_*` / `wx*` token。
- 修复播放/暂停按钮手动恢复播放失效：手动点击路径现在会在用户点击同步栈内先调用 `audio.play()`，再恢复音频分析器，避免错过浏览器用户激活窗口。
- 修复播放准备阶段的栈溢出会拖死控制台的问题：歌词预置、封面加载、喜欢状态、Home 状态、listen session 和 beatmap / DJ 视觉预热均改为隔离执行，任何非音频链路异常不再阻断真实切歌。
- `playQueueAt` 增加播放阶段日志；若后续仍出现异常，控制台会标明 `source-url`、`visual-prep`、`audio-start` 等阶段，界面不再直接弹出 `Maximum call stack size exceeded`。
- 单曲循环的 `ended` 回调改为异步再入，避免浏览器在音频结束事件中同步重进 `playQueueAt` 引发递归风险。
- 修复天气电台 / 歌单播放链路的控制台递归风险：不再运行时重写播放、上一首、下一首按钮的 `onclick`，避免 `Maximum call stack size exceeded`。
- 切歌开始时立即刷新队列高亮和迷你队列计数；即使浏览器拦截自动播放，标题、时长、队列当前项也保持同步。
- Home 天气电台主按钮和天气卡片统一到同一启动入口，并增加启动 busy 锁，避免重复点击并发开播。
- 修复右键唤起 3D 歌单架时误唤出播放器控制台：歌单架打开期间会硬隐藏底部控制台，并阻止热区/播放态 reveal。

- 隔离 3D 歌单架刷新/关闭异常：歌单 API 成功后不再因为 shelf 重建或 3D 详情列表绘制失败而误报“歌单加载失败”。
- 天气电台、歌单、播客、每日推荐和 3D 详情播放入口统一使用安全队列刷新；队列写入后播放器控制台会恢复可点击，不再被 UI 渲染异常拖死。
- 恢复歌单 / 天气电台自动开播为非 manual 路径，并将 `playQueueAt` 前置 UI 阶段纳入兜底，避免队列标题已变但实际音频和上下一首按钮停在旧状态。
- 天气电台音乐来源收窄为网易云搜索，并取消入队前批量播放 URL 探测，避免点击入口时请求堆积造成卡死。
- 天气电台前端复用正在进行的天气加载请求，避免 Home 后台预加载和手动启动同时打到后端。
- 自动播放被系统拦截时，播放器控制台保持可点击并提示手动继续；真正加载失败的队列项会自动跳到下一首。
- 手动下一首 / 上一首现在一定切换实际音频；单曲循环只在自然播完时重复当前曲。
- 切歌开始时立即暂停旧音频，避免出现歌词和 UI 已切走但旧歌还在播的错位。
- 修复天气电台城市经纬度缺失时落到 `(0,0)` 的问题，并在 Open-Meteo 超时/失败时返回临时电台队列。
- 歌单曲目加载增加 `playlist_track_all -> playlist_detail` 降级，减少歌单加载失败。
- 播放队列写入后强制恢复底部控制台可交互状态，避免天气电台或歌单开播后控制台残留不可点击。
- 启动页退出后补 Home 恢复安全网，避免首页隐藏在空场背景后。

- 发布 Mineradio 首个正式版本，GitHub 主页和安装包版本统一为 `1.0.0`。
- 首页正式版包含天气电台、每日推荐、私人电台、继续听、听歌画像和我的歌单入口。
- 天气电台接入 Open-Meteo，并优先混入每日推荐和私人推荐曲库，过滤 AI / 白噪音 / 雨声助眠等低质结果。
- 修复窗口模式下主页 6 张主卡片在紧凑尺寸中过早堆叠、挤出首屏的问题。
- 保持启动后 Wallpaper 银河背景干净显示，播放后切换到 Emily / 默认播放态视觉。

## v0.9.13

- 开场动画升级为 WebGL 光流线场，融合 `ShipSwiftAnimatedLoop` 的 RGB 分通道、Warp 流动和高亮线场质感。
- 移除刻意的环形/花瓣式爆点，改为更自然的斜向流线相位变化，让启动过程更帅气但不突兀。
- 开场动画播放完成后不再自动进入主页，会停留在“点击进入”状态；点击任意位置或按 Enter/空格后进入 Home。
- 保留 2D canvas fallback，WebGL 不可用时仍能显示启动页。
- 生成 `0.9.12 -> 0.9.13` 快速补丁，用于已安装用户轻量更新。

## v0.9.9

- 统一当前版本号为 `0.9.9`
- 接入 GitHub Releases 更新检测配置
- 增加更新弹窗与更新下载任务接口
- 增加 QQ 音乐与网易云音乐双平台体验方向
- 优化电影镜头节奏分析与 DJ 视觉模式
- 增加自定义封面、自定义歌词、歌词布局和视觉控制相关能力

## 说明

这是首次公开发布前的整理版本。更早版本属于本地迭代记录，暂不展开。
