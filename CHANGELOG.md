...
- 新增 YouTube Music 独立来源搜索，支持来源页内的综合、歌曲、歌单和频道分类，以及详情与分页；点击视频会在系统浏览器打开 YouTube Music。
- 默认综合搜索仅汇总网易云和 QQ 音乐；YouTube Data API 只在用户明确选择 YouTube Music 来源并提交查询后请求，同时增加 10 分钟成功缓存、并发合并和请求限流。
- 网易云搜索增加综合结果视图，可同时浏览单曲、歌手、专辑和歌单，并保留各分类的完整分页与播放操作。
- 新增 Google 官方 OAuth 登录，从三平台登录或账号弹窗直接跳转系统浏览器授权；支持取消、刷新和退出，用户令牌通过 Windows DPAPI 加密保存。
- 登录 YouTube 后可浏览我的歌单、喜欢的视频和订阅频道，并查看歌单或频道详情；选择视频时仍由系统浏览器打开 YouTube Music。
- YouTube 内容不写入 Mineradio 播放队列，也不接管进度、音量、媒体键或跨启动恢复；移除后台隐藏的 IFrame 播放方式。
- 统一网易云、QQ 音乐和 YouTube 的登录与账号界面，移除旧双账号状态和重复的设置“服务”页；来源标识改用完整平台名称。
- 正式 Windows 构建会注入 Google 桌面 OAuth 公共客户端配置，缺少配置时直接终止构建，避免发布无法登录 YouTube 的安装包。
