...
function aesCtrDecrypt(key, iv, encrypted) {
  if (!Buffer.isBuffer(key) || key.length !== 16 || !Buffer.isBuffer(iv) || iv.length !== 16) {
    throw new Error('AES-CTR key and IV must both be 16 bytes.')
  }
...
function parseStsz(data) {
  if (!Buffer.isBuffer(data) || data.length < 12) {
    throw new Error('Invalid stsz atom.')
  }
  ...
  if (12 + count * 4 > data.length) {
    throw new Error('Truncated stsz sample table.')
  }
...
function parseStsc(data) {
  if (!Buffer.isBuffer(data) || data.length < 8) {
    throw new Error('Invalid stsc atom.')
  }
  const entryCount = data.readUInt32BE(4)
  if (entryCount === 0 || entryCount > 100000 || 8 + entryCount * 12 > data.length) {
    throw new Error('Invalid stsc entry table.')
  }
  const entries = []
...
function parseSenc(data) {
  if (!Buffer.isBuffer(data) || data.length < 8) {
    throw new Error('Invalid senc atom.')
  }
  const count = data.readUInt32BE(4)
  if (count === 0 || count > 2000000 || 8 + count * 8 > data.length) {
    throw new Error('Invalid senc sample table.')
  }
  const ivs = []
  let position = 8
