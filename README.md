...
当前稳定安装版为 `v1.4.0`。Windows 安装包仅通过本仓库 Releases 发布。

`v1.4.0` 完善搜索、专辑、队列、歌词、播客和本人歌单管理，并重排 Home 与设置体验。完整变更见 [CHANGELOG.md](./CHANGELOG.md)。
...
- 基于节奏的电影镜头视觉系统
- 面向长播客和 DJ 曲目的专属视觉模式
- 歌词舞台、自定义歌词、双语/罗马音副行、网易云 YRC 与 QQ QRC 逐字高亮
- 自定义专辑封面上传与裁剪
- 右键唤起 3D 歌单架，支持歌单队列浏览
- 网易云音乐账号、搜索、歌单、播客、每日推荐和私人 FM 接入
- 网易云与 QQ 音乐搜索支持分页、加载更多、单来源失败重试、播放全部、批量入队和歌曲快捷操作
- 支持专辑详情、歌手专辑，以及本人普通网易云歌单的重命名、删除、移除歌曲和曲序同步
- 播放队列支持跨启动恢复、拖动/键盘排序、多选移除、撤销和保存为网易云歌单
...
Windows 用户可从 [GitHub Releases](https://github.com/dh666i/Mineradio/releases/tag/v1.4.0) 下载带 `1.4.0` 版本号的安装包，并使用同一 Release 中的 SHA256 文件核对摘要。
